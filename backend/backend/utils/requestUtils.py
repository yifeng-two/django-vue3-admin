...
LastEditors: yifeng
LastEditTime: 2022-10-07 20:47:49
...
def get_ip_analysis(ip):
    ...
    if ip != 'unknown' and ip:
        if getattr(settings, 'ENABLE_LOGIN_ANALYSIS_LOG', True):
            try:
                res = requests.get(url='https://ip.django-vue-admin.com/ip/analysis',
                                   params={"ip": ip},
                                   timeout=5)
...
def save_login_log(request):
    ...
    :return:
    """
    login_log_enbale = getattr(settings, 'LOGIN_LOG_ENABLE', None) or False
