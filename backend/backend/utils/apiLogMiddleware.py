'''
Author: yifeng
Date: 2022-10-07 14:43:17
LastEditors: yifeng
LastEditTime: 2022-10-07 19:50:28
Description: 日志 django中间件
'''
...
from backend.utils.requestUtils import (get_request_user, get_request_ip, get_request_data,
                                        get_request_path, get_os, get_browser, get_verbose_name)
...
class ApiLogMiddleware(MiddlewareMixin):
    ...
    def __handle_response(self, request, response):
        ...
        info = {
            ...
            'status': True if response.data.get('code') in [
                2000,
            ] else False,
            'json_result': {
        ...
        operation_log, creat = OperationLog.objects.update_or_create(defaults=info,
                                                                     id=self.operation_log_id)
        if not operation_log.request_modular and settings.API_MODEL_MAP.get(
                request.request_path, None):
