...
class CoreInitialize:
    ...
    def __init__(self, reset=False, creator_id=None, app=None):
        ...
        self.app = app or ''
        self.request.user = User.objects.order_by('create_datetime').first()

    def init_base(self, Serializer, unique_fields=None):
        model = Serializer.Meta.model
        path_file = os.path.join(
            apps.get_app_config(self.app.split('.')[-1]).path, 'fixtures',
        ...
        for ele in data:
            ...
            for key, m2m in m2m_dict.items():
                ...
                if m2m and len(m2m) > 0 and m2m[0]:
                    exec(f"""if object.{key}: \n"""
                         + f"""values_list = object.{key}.all().values_list('id', flat=True) \n"""
                         + f"""values_list = list(set(list(values_list) + {m2m})) \n"""
                         + f"""object.{key}.set(values_list) \n""")
